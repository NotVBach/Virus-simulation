...
global {
	...
	float death_rate <- 0.01; // 1% Death Ratio
...
grid district width: 5 height: 4 {
	...
	reflex update_lockdown when: is_locked_down {
		if (time - lockdown_start_time) >= 14 #days {
...
species people skills:[moving] {		
	...
	reflex recover_or_die when: is_infected and !is_recovered {
		if (time - infection_time) >= 10 #days {
			if flip(0.01) { nb_dead <- nb_dead + 1; do die; }
			else {
...
// --- EXPERIMENT BLOCK WITH DUAL OUTPUTS ---
experiment main type: gui {
	output {
		...
		// 1. LOCKDOWN MAP (With Districts)
		display "Lockdown Scenario Map" {
		...
		// 2. BASE SIMULATION MAP (Without Districts)
		display "Base Simulation Map" {
			// Note: We simply do not draw the 'district' grid here
			species road aspect: geom;
			species building aspect: geom;
			species people aspect: circle;			
		}
		...
		// 3. COMBINED CHART
		display "Comparative Epidemic Curves" {
			chart "SIRD Population Tracking" type: series {
		...
		// 4. DISTRICT STRAIN CHART
		display "Intervention Strain" {
			chart "Policy Intensity" type: series {
				data "Locked Districts" value: nb_districts_locked color: #blue;
				data "Active Infections" value: nb_infected / 10 color: #orange; // Scaled for comparison
			}
		}
	}
}